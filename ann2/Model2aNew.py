...
# =================================================
        ...
        self.Portfolio = numpy.loadtxt(self.AnnuityName)
        qx = numpy.loadtxt('qx_ER2000_2O.dat')
        qy = numpy.loadtxt('qy_ER2000_2O.dat')
        fx = numpy.loadtxt('fx_ER2000_2O.dat')
        fy = numpy.loadtxt('fy_ER2000_2O.dat')
# =================================================
        import markovlv
        self.hm = markovlv.ANNUITYLV()
        self.hf = markovlv.ANNUITYLV()
        for j in range(qx.shape[0]):
            dTemp = self.hm.dSetQx(long(qx[j,0]), qx[j,1])
        for j in range(fx.shape[0]):
            dTemp = self.hm.dSetFx(long(fx[j,0]), fx[j,1])
        for j in range(2500):
        ...
        for j in range(qy.shape[0]):
            dTemp = self.hf.dSetQx(long(qy[j,0]), qy[j,1])
        for j in range(fy.shape[0]):
            dTemp = self.hf.dSetFx(long(fy[j,0]), fy[j,1])
        for j in range(2500):
...
# Init Person i
           ...
           if sex == 1: 
               ...
               h = self.hf
           age = long(self.Portfolio[i,0])
           r   = self.Portfolio[i,1]
#           print sex, age, r, self.Portfolio[i,3]
           h.vSetStopTime(long(age))
           h.vSetSAge(long(self.Portfolio[i,3]))  
           u = u + r * h.dGetDK(age)
...
# Get DK and CF Person i
    ...
    def CalcSim(self):
        ...
        for k in range(self.NrSim):
            XOmega = numpy.exp(self.mu * numpy.ones((100,1), dtype='f') + self.sigma * numpy.random.randn(100,1))

            kktemp = 1.0
            
            for j in range (100):
                kktemp *= XOmega[j]
	        dTemp = self.hm.dSetRelativeQxForTime(long(j), float(kktemp))
	        dTemp = self.hf.dSetRelativeQxForTime(long(j), float(kktemp))
...
#############################################################################################################    
        ...
        numpy.savetxt(outFile,s.CF_Sim.transpose(), fmt='%.1f', delimiter=';')
        outFile.close()
